def print_all_subsequences(X, Y, C):
    ...
    LCS = [''] * (C[m][n] + 1)   #LCS=current_subsequence,1 liner replacement of (id = C[m][n] >>lcs = [''] * (id + 1) >>lcs[id] = '\0')
    ...
    def find_all_subsequences(i, j, id):
        ...
        if C[i][j - 1] >= C[i - 1][j]:
            find_all_subsequences(i, j - 1, id) #''.join(LCS[id:]): This part joins the elements of the LCS list from index id to the end (: indicates the end of the list) into a single string.
